 ...
 * @param dof 机器人机器人自由度
 * @return int 正确返回值:_move_finish(0),_move_stop(7),  异常返回值: 机器人正在运行_move_run_joint(1),_move_run_line(2),_move_run_cricle(3),_move_run_helical(4),_move_run_bspline(5),_move_run_zone(6),_move_run_zone_finish(8)；运动状态错误_move_error(-1)
 */
 ...
 * @return int 0: 成功, 其他：失败
 */
extern int calibration_user_data(int data_index);
 ...
 * @return int 0: 成功, 其他：失败
 */
extern int calibration_wobj_data(int data_index);
 ...
 * @return int 0: 成功, 其他：失败
 */
extern int calibration_wobj(int robhold, int ufmec, int ufprog, const char* user_name,  const char* wobj_name,wobj* restul_wobj);
