...
#define ROBOT_DATA_NAME_LEN_MAX 50
 ...
 * @return int 正确返回0，错误返回其他(-1:指定名字无效)
 */
int writeuser(const char* U, robpose* us);
 ...
 * @return int 正确返回0，错误返回其他(-1:指定名字无效)
 */
int writePayLoad(PayLoad* payload, const char* payloadname);
 ...
 * @return int 正确返回0，错误返回其他(-1:指定名字无效)
*/
int deletePayLoad(const char* payloadname);
...
//数据变量宏定义
typedef  robjoint* robjointptr;
typedef  robpose* robposeptr;
typedef  speed* speedptr;
typedef  zone* zoneptr;
typedef  tool* toolptr;
typedef  wobj* wobjptr;
